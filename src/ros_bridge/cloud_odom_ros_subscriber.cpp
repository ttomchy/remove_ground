...
void PrintMsgStats(const sensor_msgs::PointCloud2ConstPtr& msg) {//消息的状态类型
...
CloudOdomRosSubscriber::CloudOdomRosSubscriber(NodeHandle* node_handle,
  ...
  std::cout<<"Now it runs in the CloudOdomRosSubscriber::CloudOdomRosSubscriber( function !!"<<std::endl;
...
void CloudOdomRosSubscriber::StartListeningToRos() {
  std::cout<<"Now it runs in theCloudOdomRosSubscriber::StartListeningToRos function !!"<<std::endl;
...
void CloudOdomRosSubscriber::Callback(const PointCloud2::ConstPtr& msg_cloud,
  ...
  std::cout<<"Now it runs in the CloudOdomRosSubscriber ::call back function !!"<<std::endl;
...
void CloudOdomRosSubscriber::CallbackVelodyne(
  ...
  std::cout<<"Now it runs in the CloudOdomRosSubscriber::CallbackVelodyne function !!"<<std::endl;
  PrintMsgStats(msg_cloud);
  Cloud::Ptr cloud_ptr = RosCloudToCloud(msg_cloud);
  cloud_ptr->InitProjection(_params);
  ShareDataWithAllClients(*cloud_ptr);
...
Pose CloudOdomRosSubscriber::RosOdomToPose(const Odometry::ConstPtr& msg) {
  std::cout<<"Now it runs in the CloudOdomRosSubscriber::RosOdomToPose function !!"<<std::endl;
  Pose pose;
...
Cloud::Ptr CloudOdomRosSubscriber::RosCloudToCloud(
  ...
  std::cout<<"Now it runs in the CloudOdomRosSubscriber::RosCloudToCloud function !!"<<std::endl;
