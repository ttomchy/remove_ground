...
float LineDistDiff::DiffAt(const PixelCoord& from, const PixelCoord& to) const {
 // std::cout<<" Now it runs in the DiffAt function  param is PixelCoord& from, const PixelCoord& to !"<<std::endl;
...
void LineDistDiff::PreComputeAlphaVecs() {
  //std::cout<<" Now it runs in the PreComputeAlphaVecs function   !"<<std::endl;
...
float LineDistDiff::ComputeAlpha(const PixelCoord& current,
  ...
  //std::cout<<" Now it runs in the ComputeAlpha function !"<<std::endl;
  if ((current.col == 0 &&
...
LineDistDiffPrecomputed::LineDistDiffPrecomputed(const cv::Mat* source_image,
                                                 const ProjectionParams* params)
    : AbstractDiff{source_image}, _params{params} {
  //std::cout<<" Now it runs in the LineDistDiffPrecomputed function !"<<std::endl;
  PreComputeAlphaVecs();
  PreComputeLineDists();
}
...
float LineDistDiffPrecomputed::DiffAt(const PixelCoord& from,
  ...
  //std::cout<<" Now it runs in the DiffAt function  param is const PixelCoord& from, const PixelCoord& !"<<std::endl;
  int16_t row = 0;
  int16_t col = 0;
...
cv::Mat LineDistDiffPrecomputed::Visualize() const {
  ...
  //std::cout<<" Now it runs in the LineDistDiffPrecomputed::Visualize() function  !"<<std::endl;
...
void LineDistDiffPrecomputed::PreComputeLineDists() {
 // std::cout<<" Now it runs in the PreComputeLineDists function  !"<<std::endl;
...
float LineDistDiffPrecomputed::GetLineDist(float alpha, float current_depth,
 ...
 // std::cout<<" Now it runs in the GetLineDist function  !"<<std::endl;
  float d1 = std::max(current_depth, neighbor_depth);
  float d2 = std::min(current_depth, neighbor_depth);
  float beta = fabs(std::atan2((d2 * sin(alpha)), (d1 - d2 * cos(alpha))));
  return d1 * sin(beta);
