...
AngleDiff::AngleDiff(const cv::Mat* source_image,
   ...
   std::cout<<" Now it run in the anglediff.cpp before tit is in the  diff_factory.h"<<std::endl;
  PreComputeAlphaVecs();
}

float AngleDiff::DiffAt(const PixelCoord& from, const PixelCoord& to) const {
  //std::cout<<" Now it runs in the DiffAt function ! it is in the angle_diff.cpp "<<std::endl;
...
void AngleDiff::PreComputeAlphaVecs() {
  std::cout<<" Now it runs in the PreComputeAlphaVecs function in the anglediff.cpp!"<<std::endl;
  ...
  std::cout<<" Now it is finished in the PreComputeAlphaVecs function in the anglediff.cpp!"<<std::endl;
}

float AngleDiff::ComputeAlpha(const PixelCoord& current,
                              const PixelCoord& neighbor) const {
  //std::cout<<" Now it runs in the ComputeAlpha function it is in the anglediff.cpp!"<<std::endl;
  if ((current.col == 0 &&
...
AngleDiffPrecomputed::AngleDiffPrecomputed(const cv::Mat* source_image,
                                           const ProjectionParams* params)
    : AbstractDiff{source_image}, _params{params} {
  std::cout<<" Now it runs in the AngleDiffPrecomputed function it is in the angle_diff.cpp !"<<std::endl;
  PreComputeAlphaVecs();
...
void AngleDiffPrecomputed::PreComputeAlphaVecs() {
  std::cout<<" Now it runs in the PreComputeAlphaVecs function it is in the angle_diff.cpp  !"<<std::endl;
  _row_alphas.reserve(_params->rows());
  for (size_t r = 0; r < _params->rows() - 1; ++r) {
    _row_alphas.push_back(
        fabs((_params->AngleFromRow(r + 1) - _params->AngleFromRow(r)).val()));
...
void AngleDiffPrecomputed::PreComputeBetaAngles() {
 std::cout<<" Now it runs in the PreComputeBetaAngles function it is in the angle_diff.cpp  !"<<std::endl;
...
float AngleDiffPrecomputed::GetBeta(float alpha, float current_depth,
  ...
  std::cout<<" Now it runs in the GetBeta functionit is in the angle_diff.cpp   !"<<std::endl;
  float d1 = std::max(current_depth, neighbor_depth);
  float d2 = std::min(current_depth, neighbor_depth);
  float beta = std::atan2((d2 * sin(alpha)), (d1 - d2 * cos(alpha)));
  return fabs(beta);
