...
class ProjectionParams {
 public:
  using Ptr = shared_ptr<ProjectionParams>;// define a new name of shared_ptr<ProjectionParams>
  using ConstPtr = const shared_ptr<const ProjectionParams>;
  enum class Direction { HORIZONTAL, VERTICAL };//<direction ,horizontal,vertical>
 ...
 private:
  std::vector<Radians> FillVector(const Radians& start_angle,
                                  ...
                                  const Radians& step); //FillVector start_angle ,end_angle, step;
  ...
  std::vector<Radians> _row_angles;//  why does it has the _row_angles parameters ?
