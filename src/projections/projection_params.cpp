...
void ProjectionParams::SetSpan(const Radians& start_angle,
  ...
  std::cout<<" Now it runs in the const SetSpan function params is const Radians& start_angle, const Radians& end_angle, const Radians& step, !"<<std::endl;
  //std::cout<< " 好像并没有进入到这里面来 projection parameter 里面！！！"<<std::endl;
  switch (direction) {
    case Direction::HORIZONTAL: // HORIZONTAL -180_deg ---180_deg 
      _h_start_angle = start_angle;
      _h_end_angle = end_angle;
      _h_span = Radians::Abs(_h_end_angle - _h_start_angle); //
      _col_angles = FillVector(start_angle, end_angle, step);// step is defined when the function is referenced 
      break;
    case Direction::VERTICAL://VERTICAL -15_deg ---15__deg;
  ...
  FillCosSin();// it seems that this function does nothing .
}

vector<Radians> ProjectionParams::FillVector(const Radians& start_angle,
                                             const Radians& end_angle,
                                             const Radians& step) {
  std::cout<<" Now it runs in the FillVector function params is  const Radians& start_angle,  const Radians& end_angle,   const Radians& step!"<<std::endl;
  vector<Radians> res;
...
void ProjectionParams::SetSpan(const Radians& start_angle,
                               ...
                               const ProjectionParams::Direction& direction) {
  std::cout<<" Now it runs in the FillVector function param is  Radians& start_angle,  Radians& end_angle, const int num_bins,  const ProjectionParams::Direction& direction !"<<std::endl;
  Radians step = (end_angle - start_angle) / num_bins;
  this->SetSpan(start_angle, end_angle, step, direction);
}

bool ProjectionParams::valid() {
  //std::cout<<" Now it runs in the ProjectionParams::valid() function !"<<std::endl;
...
std::unique_ptr<ProjectionParams> ProjectionParams::VLP_16() {
  ...
  params.SetSpan(-180_deg, 180_deg, 870, Direction::HORIZONTAL);//why 870?
  ...
  return mem_utils::make_unique<ProjectionParams>(params);// it seems that i have met it before 
}
