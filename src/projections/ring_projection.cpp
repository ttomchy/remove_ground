...
CloudProjection::Ptr RingProjection::Clone() const {
  std::cout<<" Now it runs in the const RingProjection::Clone() function !"<<std::endl;
...
RichPoint RingProjection::UnprojectPoint(const cv::Mat& image, const int row,
                                         const int col) const {
  //std::cout<<" Now it runs in the const UnprojectPoint function and it is in the ring_projection.cpp!"<<std::endl;
